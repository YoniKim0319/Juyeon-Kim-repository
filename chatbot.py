...
def create_chat_bubble(text, is_user=True):
    ...
    color = '#f1f3f4' if is_user else '#a1c9f1'
    st.markdown(
        f"<div style='background-color: {color}; padding: 10px 15px; border-radius: 15px; width: fit-content; "
        f"max-width: 80%; margin: 5px; float: {align}; clear: both;'>"
        f"{text}</div>", unsafe_allow_html=True
    )

# 스트림릿 페이지 설정
st.set_page_config(page_title="Recycling FAQ Bot", page_icon="🌍", layout="wide")
...
def get_answer(question):
    ...
    for keyword, answer in recycling_faq.items():
...
# 앱 타이틀 및 설명
st.markdown("""
    <h1 style='text-align: center; color: navy;'>♻️ 쓰레기 분리수거 FAQ 챗봇</h1>
    <p style='text-align: center;'>쓰레기 분리수거에 대한 궁금증을 해결해드립니다. 아래에 질문을 입력해주세요.</p>
